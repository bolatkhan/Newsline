...
class FeedParser: NSObject, NSXMLParserDelegate {
    ...
    func parseEndOfRSS2Element(elementName: String, qualifiedName qName: String!) {
        ...
        // media:content
        else if self.currentPath == "/rss/channel/item/media:content" {
            ...
            if content != nil && type != nil {
                let feedMedia = FeedMedia(url: content!, type: type!)
                self.currentFeedItem.feedMedia.append(feedMedia)
            }
        }
...
//            let length:Int? = Int((self.currentElementAttributes?["length"] as? String)!)
            if content != nil && type != nil && 0 != nil {
                let feedEnclosure = FeedEnclosure(url: content!, type: type!, length: 0)
                self.currentFeedItem?.feedEnclosures.append(feedEnclosure)
            }
        }
            
            // media:group media:thumbnail
        else if self.currentPath == "/rss/channel/item/media:group/media:thumbnail" {
            let content: String? = self.currentElementAttributes?["url"] as? String
            if content != nil {
                self.currentFeedItem?.feedImage = content
            }
        }
            
            // media:thumbnail
        else if self.currentPath == "/rss/channel/item/media:thumbnail" {
            let content: String = self.currentElementAttributes?["url"] as! String
            if !content.isEmpty {
                self.currentFeedItem?.feedImage = content
            }
        ...
        // image
        else if self.currentPath == "/rss/channel/item/image/url" {
            self.currentFeedItem?.feedImage = self.currentElementContent
        }
