...
class CenterViewController: MainViewController, FeedParserDelegate {
    ...
    override func viewDidLoad() {
        ...
        if self.mode != "singleapp" {
            ...
            self.hamburgerButtonCloseSmall.addTarget(self, action: #selector(CenterViewController.leftMenuButtonPressed(_:)), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        self.refreshControl.addTarget(self, action: #selector(CenterViewController.refresh(_:)), forControlEvents: UIControlEvents.ValueChanged)
        ...
        if (!Defaults.hasKey("firstNewsSource")) {
            if self.SourceData["ParseURL"]?.length == 0 && self.SourceData["CatIdentifier"]?.length == 0 {
                ...
                self.FetchTimer = NSTimer.scheduledTimerWithTimeInterval(1.5, target: self, selector: #selector(CenterViewController.requestContent), userInfo: nil, repeats: false)
            }
        }
        ...
        if self.mode != "singleapp" {
            ...
            NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(CenterViewController.LeftMenuAnimatedButton(_:)), name: "LeftMenuChangedNotification", object:nil)
        }
        // kaldirilacak - finish
        NSNotificationCenter.defaultCenter().removeObserver(self, name: "LeftMenuSelectedItem", object:nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(CenterViewController.LeftMenuSelected(_:)), name: "LeftMenuSelectedItem", object:nil)
        NSNotificationCenter.defaultCenter().removeObserver(self, name: "StopParsingTimer", object:nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(CenterViewController.StopParsingTimeNotification(_:)), name: "StopParsingTimer", object:nil)
        //NSNotificationCenter.defaultCenter().removeObserver(self, name: "StopParsingTimer", object:nil)
...
/*
    ...
    func requestContent() {
        ...
        if CatID == "native-last-news" {
            ...
            self.FetchLoadingTimer = NSTimer.scheduledTimerWithTimeInterval(35.4, target: self, selector: #selector(CenterViewController.StopParsingTime), userInfo: nil, repeats: false)
            if self.ProgressHUD.hidden {
                self.ProgressHUD.showInView(self.view)
            }
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        if (Defaults.hasKey("firstNewsSource")) {
            ...
            self.SourceData["CatIdentifier"] = "first-news-sources"
            Defaults.remove("firstNewsSource")
            self.FetchTimer = NSTimer.scheduledTimerWithTimeInterval(0.3, target: self, selector: #selector(CenterViewController.requestContent), userInfo: nil, repeats: false)
        }
    }
