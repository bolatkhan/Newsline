...
class MainLeftMenuViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, FeedParserDelegate {
    ...
    override func viewDidLoad() {
        ...
        let MBackgroundColor = self.AppColors["leftmenubackground"] as! String //"#F4F4F4"
        ...
        let TopText = UILabel(frame: CGRectMake(0, 20, 200, 45))
        TopText.text=localizedString("contenttitle")
        //TopText.backgroundColor = UIColor.blackColor()
        TopText.font = Font.menuFont
        ...
        self.settingsButton.addTarget(self, action: #selector(MainLeftMenuViewController.OpenSettingsView(_:)), forControlEvents: UIControlEvents.TouchUpInside)
        self.settingsButton.hidden = true
        
        self.view.addSubview(self.settingsButton)
        
        self.editSourceButton = UIButton(frame: CGRect(x: 205, y: 20, width: 45, height: 45))
        self.editSourceButton.setImage(UIImage(named: "edit"), forState: .Normal)
        //self.settingsButton.backgroundColor = UIColor(rgba: "#32374B")
        self.editSourceButton.backgroundColor = UIColor(rgba: MBackgroundColor)
        self.editSourceButton.imageEdgeInsets = UIEdgeInsetsMake(10,10,10,10);
        //self.settingsButton.addTarget(self, action: "OpenCountryView:", forControlEvents: UIControlEvents.TouchUpInside)
        self.editSourceButton.addTarget(self, action: "startEditMode:", forControlEvents: UIControlEvents.TouchUpInside)
        
        self.view.addSubview(self.editSourceButton)
        ...
        constrain(self.tableView,self.SearchBox) { tview,searchBox in
            tview.top == searchBox.bottom
            tview.width == (tview.superview?.width)!
            tview.bottom == (tview.superview?.bottom)!
        }

        constrain(self.editSourceButton,self.addSourceButton,TopText) { eview,aview,ttext in
            //align(top: eview,aview)
            //eview.right >= (eview.superview?.right)!
            aview.right == eview.left
            aview.top == eview.top
            aview.height == 45
            aview.width == 45
            ttext.top == aview.top
            ttext.right == aview.left
            ttext.left == (ttext.superview?.left)!+10
            ttext.height == eview.height
        }
        ...
        if(self.mode == "menuapp" || self.mode == "singleapp") {
            self.settingsButton.hidden = true
            self.addSourceButton.hidden = true
            self.editSourceButton.hidden = true
        }
    }
    ...
    func OpenSettingsView(sender: UIButton!) {
        ...
        nsettingsController.navigationBar.titleTextAttributes = titleDict as? [String : AnyObject]
    ...
    func keyboardWillBeHidden (notification: NSNotification) {
        ...
        if let keyboardSize = (notification.userInfo?[UIKeyboardFrameBeginUserInfoKey] as? NSValue)?.CGRectValue() {
            self.tableView.frame.size.height = self.view.frame.size.height-(self.SearchBox.frame.size.height+20)
    ...
    func searchBarShouldBeginEditing(searchBar: UISearchBar) -> Bool {
        ...
        let topView: UIView = self.SearchBox.subviews[0] 
    ...
    func startEditMode(sender: UIButton!) {
        if self.TableState == STATE.VIEW {
            ...
            //
            self.editSourceButton.frame.origin.x = self.view.bounds.size.width-self.editSourceButton.bounds.width
            self.editSourceButton.setImage(UIImage(named: "edithover"), forState: .Normal)
        } else if self.TableState == STATE.SEARCH || self.TableState == STATE.EDIT {
            self.minimizeLeftMenu()
    ...
    func stopSearchMode() {
        ...
        */
        ...
        //self.editSourceButton.frame.origin.x = 100
        self.editSourceButton.frame.origin.x = self.view.bounds.size.width-self.editSourceButton.bounds.size.width
        //self.SearchBox.text = ""
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell:LeftMenuViewCell = self.tableView.dequeueReusableCellWithIdentifier("MENU") as! LeftMenuViewCell
        ...
        if self.TableState == STATE.VIEW {
            ...
            let res = eqDictionary(self.DatabaseItems, key: "name", value: self.items[indexPath.row]["name"]!)
        ...
        bgColorView.backgroundColor = UIColor(rgba: (self.AppColors["leftmenuselected"] as! String))
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if self.TableState == STATE.VIEW {
            ...
            let name = self.items[indexPath.row]["name"] as! String
            let url = self.items[indexPath.row]["url"] as! String
            let identifier = self.items[indexPath.row]["identifier"] as! String
            let favicon = self.items[indexPath.row]["favicon"] as! String
            if self.tableView.cellForRowAtIndexPath(indexPath)?.accessoryType == UITableViewCellAccessoryType.None {
                ...
                if result != 0{
                    let deleteIndex = eqDictionaryGetKey(self.items, key: "name", value: name)
                    if deleteIndex != -1 {
        ...
        if deleteresult != 0 {
            ...
            if !self.items.isEmpty {
                for row in self.items {
                    ...
                    db.execute("INSERT OR REPLACE INTO mylist (name,url,orderlist,identifier,favicon) VALUES (?,?,?,?,?)", parameters:[nname,nurl,order,identifier,favicon])
                    order++
                }
    ...
    func OpenEditTable() {
        self.evo_drawerController?.openDrawerGestureModeMask = OpenDrawerGestureMode.PanningNavigationBar
        self.evo_drawerController?.closeDrawerGestureModeMask = CloseDrawerGestureMode.PanningNavigationBar
