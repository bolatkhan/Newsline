...
class CenterTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if device.isPad() {
            ...
            if item.sname == "Forbes"{
                FIRAnalytics.logEventWithName("forbes", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "The-Steppe"{
                FIRAnalytics.logEventWithName("thesteppe", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Zakon.kz"{
                FIRAnalytics.logEventWithName("zakon", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Diapazon"{
                FIRAnalytics.logEventWithName("diapazon", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Khabar"{
                FIRAnalytics.logEventWithName("khabar", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "NUR.kz"{
                FIRAnalytics.logEventWithName("nurkz", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Tengrinews"{
                FIRAnalytics.logEventWithName("tengrinews", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Informburo"{
                FIRAnalytics.logEventWithName("informburo", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Today.kz"{
                FIRAnalytics.logEventWithName("today", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
            if item.sname == "Kursiv"{
                FIRAnalytics.logEventWithName("kursiv", parameters: [
                    "name": item.sname as NSObject,
                    "full_text": item.title as NSObject
                    ])
            }
